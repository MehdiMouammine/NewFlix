	...
	<div id="mly0" style="width: 100%; background-color: #000; <?php echo $ww;?> background-position: center; background-repeat: no-repeat; background-size: cover;">
		<?php
			...
			foreach ($series as $s) {
				...
				$serie = $con->getSerie($s);
				$code="my".$serie->getIdSr();
				$attr="dark('".$code."')";
				$attr2="light('".$code."')";
				$gy= floor($i/6); 
				echo '<div style="width: 19.5%; margin-left: 0.5%; margin-top: 0.5%; margin-bottom: 0.5%; display: inline-block;">
						<a href="#mly'.$gy.'"><img src="'.$serie->getIcon().'" onmouseleave="'.$attr2.'" onmouseenter="'.$attr.'" id="'.$code.'"  onclick="chargerx('.$serie->getIdSr().','.$gy.')" style="width:100%"></a>
					 </div>';
				}
			if ($i%5!=0) {
